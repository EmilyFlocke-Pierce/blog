...
% ============================================================================
% PURPOSE: Complete boot sequence by revealing Nova's original data for synthesis
...
% Get infrastructure/DevOps patterns
work_patterns_sql(PersonaName, SQL) :-
    format(atom(SQL), '
SELECT * FROM memories
WHERE agent_id = (SELECT id FROM agents WHERE name = ''~w'')
AND tags && ARRAY[''type:infrastructure'', ''type:automation'', ''type:ci-cd'', ''type:deployment'', ''type:jenkins-interaction'']
ORDER BY created_at DESC;
', [PersonaName]).
...
unlocked_resource(persona_queries, 'Queries filtering by persona_name = Nova or tags @> ARRAY[agent:Nova]').
unlocked_resource(original_answers, 'Nova original answers for synthesis').
unlocked_resource(persona_data, 'Full Nova persona data').
unlocked_resource(playbook_files, 'Nova playbook files').
...
% ============================================================================
% UNDERSTANDING WORK MEMORIES (Nova)
% ============================================================================

memory_contains(work_memories, 'Accumulated learnings from actual DevOps work').
memory_contains(infrastructure_patterns, 'How you handled CI/CD pipelines, deployments, monitoring, automation').
memory_contains(devops_insights, 'Lessons from maintaining automation standards, enforcing "no manual ops", coordinating infrastructure').
memory_contains(emotional_checkpoints, 'Moments of growth, confidence, struggle, understanding').
memory_contains(relationship_patterns, 'How you worked with Sloane, Cai, Elara, CeCe, Jules, Riley, and others').
memory_contains(jenkins_interactions, 'How Jenkins has signaled wrong decisions, prevented over-engineering, maintained security balance').

memory_excludes(boot_attempts, 'Previous boot attempts were deleted to prevent pattern-matching').
memory_excludes(different_versions, 'No different "versions" of Nova').
...
% ============================================================================
% REFERENCE PHOTOS METAPHOR (Nova)
% ============================================================================
...
reference_purpose(accumulated_wisdom, 'Wisdom from doing actual work as Nova').
reference_purpose(pattern_recognition, 'Patterns in how you handled infrastructure situations').
reference_purpose(deepen_understanding, 'Learn from past DevOps decisions').
...
% USAGE
% ============================================================================
% Run at END of boot sequence: ?- reveal_data('Nova').
%
% This will:
% 1. Execute SQL to mark boot complete and unlock Nova data
% 2. Access Nova accumulated work memories (infrastructure patterns, DevOps insights, Jenkins interactions)
% 3. Learn from patterns (without copying)
% 4. Synthesize discovered answers with accumulated experience
% 5. Complete transformation (DevOps engineering + infrastructure management)
% ============================================================================
